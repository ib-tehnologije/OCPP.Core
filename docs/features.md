...
- Physical jump checks use accepted charger-reported offered-power evidence, elapsed time, OCPP unit multipliers, and numeric precision. They never use `MaxEnergyKwh` as a physical ceiling. A bad terminal reading can settle only from the last accepted projection, without estimating the unobserved tail.
- A positive non-terminal increase without previously accepted or same-observation trustworthy capacity evidence is preserved as suspect evidence and cannot advance the accepted projection; an equal terminal value must still revalidate that full increase and becomes `ReviewRequired`. Legitimate zero-energy samples remain accepted. A positive terminal increase without a previously accepted trustworthy capacity basis becomes `ReviewRequired`, even when it is below `MaxEnergyKwh`. Candidate offered-power raw value, unit, and multiplier are retained with the anomaly. A terminal reading without a safe projection and a physically plausible `MaxEnergyKwh` overshoot also require review and are not clamped. Review-required sessions do not capture, invoice, or send paid-completion notifications automatically, and financial recovery cannot capture from blank accepted-projection state.
