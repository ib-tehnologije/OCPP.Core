...
- A terminal reading without a safe projection or trustworthy capacity basis becomes `ReviewRequired`. A physically plausible `MaxEnergyKwh` overshoot also requires review and is not clamped. Review-required sessions do not capture, invoice, or send paid-completion notifications automatically.
