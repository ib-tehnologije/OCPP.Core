...
Customers can opt in to remembering the reusable company fields in browser local storage. The data is not an account profile, never includes reservation or payment identifiers, and is shown with a shared-device privacy warning. Stripe Checkout and PaymentIntent receive a bounded metadata copy for payment reconciliation; e-racuni issuance uses the durable reservation snapshot as its source of truth.
