...
Confirmed company invoice buyer data is stored as nullable bounded columns on `ChargePaymentReservation`. Migration `AddInvoiceBuyerSnapshot` is non-destructive for existing reservations. Migration `AddInvoiceBuyerVatValidation` adds the original identifier, canonical VAT identifier, local validation status, VIES status, checked time, and bounded reference; its nullable columns do not rewrite historical rows. New public sessions must confirm the complete buyer snapshot before Stripe Checkout is created. Existing legacy R1 reservations can still build from Stripe metadata, while newly confirmed requests use the durable reservation snapshot as the invoice source of truth. The legacy buyer-data endpoint remains available for compatibility. The public status page now exposes that same endpoint as a prefilled correction form until invoice issuance; it creates no new database field or migration.

The public start and status pages do not retain reusable company-buyer details in browser storage. Submitted values survive only the ordinary server-rendered validation-error response; a new start-page visit begins with an empty buyer form. The uncached status page reads the authoritative snapshot for its reservation and includes `InvoiceBuyerConfirmedAtUtc` as the optimistic concurrency version. Identical retries are idempotent; changed requests with a missing or stale version are rejected and reload the latest snapshot. Confirmed-snapshot edits require the complete legal buyer fields even for Croatian buyers; the relaxed Croatian shape remains only for legacy first confirmation.

Foreign VAT identifiers are always normalized and structure-checked locally before Stripe. Optional VIES verification is disabled by default and can be enabled with `Payments:Vies:Enabled=true`; `Payments:Vies:TimeoutSeconds` is clamped to 1-10 seconds. A timeout, transport failure, malformed response, or VIES/member-state outage becomes `Unavailable` and must not block checkout or a pre-issuance correction. A changed canonical foreign VAT identity runs VIES again, while an unchanged identity preserves its prior result. The request contains only `countryCode` and `vatNumber`. Never add company names, addresses, billing emails, credentials, or requester identity fields to the VIES request. Generic foreign identifiers that are not marked as VAT registrations do not call VIES and retain their entered value.
...
The public buyer-data endpoint and status form lock before the invoice provider boundary whenever a submission lease is active or the durable state is `Submitting` or `ProviderUnknown`; submitted status or any external document identifier, number, or URL remains permanently locked. Failure to query invoice state also locks the form rather than assuming that no invoice exists. On relational stores, edit saves and invoice draft/lineage acquisition first take a serializable no-op update lock on the same reservation row. Invoice submission reloads the buyer snapshot under that lock and persists its lease before releasing it, so another application instance either saves first and is included in the draft or observes the durable submission barrier. Corrections after issuance or an indeterminate provider result must use support and the provider-supported reconciliation, correction, storno, or reissue process rather than mutating the reservation snapshot. The R1 request email links to the existing reservation status URL and tells the customer that self-service corrections end at issuance.

Stripe Checkout and PaymentIntent metadata are reconciliation mirrors, not the invoice source of truth. A mirror failure after the reservation save returns `UpdatedMetadataPending` only when `IStripeBuyerMetadataReconciliationJob` is durably enqueued on the configured `Hangfire:Queue` (default `payments`). If enqueueing is unavailable or fails, the response is `UpdatedMetadataRetryRequired` and directs the customer to retry or contact support instead of implying that background work is pending. Hangfire retries a queued job until both mirrors contain the latest `buyer_data_version`; operators should investigate persistent retries or a deployment without durable Hangfire storage. Repeating the same customer save also retries the mirror without changing the authoritative version.
