...
Migration `AddInvoiceSubmissionIdempotency` adds nullable `InvoiceSubmissionLog.SubmissionKey`, a filtered unique index, and nullable bounded lease identifier/expiry columns. Historical rows remain null and are not backfilled. New submit-mode attempts use the provider plus deterministic API transaction reference as their durable lineage and hold a five-minute database lease across the provider create boundary. An ordinary repeated, concurrent, or restarted completion attempt checks submitted/external local evidence first and queries the provider by the invoice draft's deterministic `orderReference`. Only the documented root-array response is recognized: one exact `orderReference` match is found, an empty root array is not found, and transport errors, object/error envelopes, non-empty unmatched arrays, missing durable identifiers, or multiple exact matches remain `ProviderUnknown` and fail closed. Explicit allowlisted invoice recovery is the exception: after the same local-history and lease gates, it replays `SalesInvoiceCreate` with the unchanged reservation-derived API transaction identifier and does not query `SalesInvoiceList`.
...
5. Run with `--execute --confirm-sha256 <digest>`. Every row is reloaded and rechecked immediately before its operation. Authorization release delegates to the existing provider ownership and `requires_capture` reconciler. Settlement capture suppresses invoice/customer-notification side effects so invoice recovery stays separately allowlisted. Before invoice lineage or provider work, invoice recovery retrieves the reservation's stored Stripe Checkout Session and requires the returned session identifier and PaymentIntent identifier to match the reservation exactly. The verified session supplies the same buyer context used by ordinary completion. Missing, unavailable, or mismatched Stripe context stops the item before `SalesInvoiceCreate`. Invoice recovery then rejects submitted or external history, acquires the existing unique lineage and durable submission lease, and replays the ordinary create request with the same deterministic API transaction identifier. It does not call `SalesInvoiceList`.
...
Ordinary completion retry lookup failures retain sanitized diagnostic structure in the existing submission audit: whether a provider request was attempted, the failure category, the nullable HTTP status, the recognized response shape, and `SalesInvoiceList` as the provider operation. Raw provider bodies, credentials, and customer identifiers are not copied into lookup-failure diagnostics. Explicit invoice recovery does not use this lookup path. A create response updates the same audit lineage to `Submitted`; any exception after the recovery provider call remains `ProviderUnknown`, retaining the stable API transaction identifier for a separately approved replay.
