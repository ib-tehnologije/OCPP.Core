...
Suspect observations are stored separately in `MeterEvidenceAnomaly` with their raw value, unit and multiplier, candidate offered-power raw value, unit and multiplier, protocol, source, timestamp, outcome, reason, and projection/capacity context. A deterministic evidence key and unique database index make exact replays idempotent while keeping differently encoded energy or power evidence distinct. Settlement, connector-Available cleanup, cleanup retry, financial recovery, and invoice entry points recheck the durable meter-evidence state.
