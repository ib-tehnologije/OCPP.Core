...
If a finite value increases at all but has neither a previously accepted physical-capacity basis nor trustworthy capacity evidence in that non-terminal observation, it does not advance the accepted projection and its raw evidence is preserved. An equal terminal value must therefore revalidate the complete unverified increase and stop at `ReviewRequired`. Zero-energy observations remain valid without capacity evidence. Invalid terminal evidence without an accepted projection also stops automated financial completion at `ReviewRequired`. Numeric precision tolerance remains limited to monotonic and capacity calculations; it cannot establish missing physical capacity. The authorization boundary does not make a smaller increase physically trustworthy. A physically plausible authorization overshoot also remains `ReviewRequired`; it is not clamped to `MaxEnergyKwh`.
