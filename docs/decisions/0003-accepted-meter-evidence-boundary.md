...
Suspect observations are stored separately in `MeterEvidenceAnomaly` with their raw value, unit and multiplier, protocol, source, timestamp, outcome, reason, and projection/capacity context. A deterministic evidence key and unique database index make exact replays idempotent while keeping differently encoded evidence distinct. Settlement, connector-Available cleanup, cleanup retry, financial recovery, and invoice entry points recheck the durable meter-evidence state.

If a finite terminal value exceeds the authorization boundary but there is no accepted physical-capacity basis, or if invalid terminal evidence has no accepted projection, automated financial completion stops at `ReviewRequired`. A physically plausible authorization overshoot also remains `ReviewRequired`; it is not clamped to `MaxEnergyKwh`.
...
- SQL Server deployments require migration `AddMeterEvidenceSafeguard`; SQLite test databases must be recreated through their existing `EnsureCreated()` workflow.
