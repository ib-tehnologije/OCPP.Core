...
4) POST `/Public/Start` calls the server API `/API/Payments/Create` with:
...
---------------------------------------------
- `POST /API/Payments/Create` (`OCPPMiddleware.HandlePaymentCreateAsync`)
...
- `POST /API/Payments/Confirm` (`HandlePaymentConfirmAsync`)
...
Stripe Webhook
--------------
- Point Stripe to `https://{server-host}/API/Payments/Webhook`.
