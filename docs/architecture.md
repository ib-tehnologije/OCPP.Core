...
8. Exceptional financial recovery is isolated in `OCPP.Core.Recovery`. A manifest allowlists exact reservations, assessors fail closed on incomplete persisted evidence, and execute mode requires the exact manifest digest. Settlement reuses the payment coordinator while suppressing customer side effects; authorization release reuses the durable reconciler. Invoice recovery dry-run remains local-only. Execute mode first retrieves the stored Stripe Checkout Session through a read-only dependency and requires exact session and PaymentIntent identity matches, preserving the ordinary completion buyer context; unavailable or conflicting context stops before invoice-provider work. Recovery then uses a unique submission lineage and a time-bounded database lease before replaying the ordinary create request with the same deterministic provider API transaction identifier.
