    ...
    public partial class OCPPMiddleware
    {
        ...
        private async Task Receive20(ChargePointStatus chargePointStatus, HttpContext httpContext, OCPPCoreContext dbContext)
        {
                            ...
                            if (result.EndOfMessage)
                            {
                                ...
                                if (match != null && match.Groups != null && match.Groups.Count >= 3)
                                {
                                    ...
                                    else if (msgIn.MessageType == "3" || msgIn.MessageType == "4")
                                    {
                                        ...
                                        {
                                            controller20.ProcessAnswer(msgIn, originalRequest);
                                        }
                                        else
                                        {
            ...
            finally
            {
                ...
                RemoveChargePointStatusIfCurrentSession(chargePointStatus, "receive20-finally");
            }
        }
        ...
        /// </summary>
        private async Task Reset20(ChargePointStatus chargePointStatus, HttpContext apiCallerContext, OCPPCoreContext dbContext, Messages_OCPP20.ResetEnumType resetType)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP20");
            ControllerOCPP20 controller20 = new ControllerOCPP20(_configuration, _logFactory, chargePointStatus, dbContext);
        ...
        private async Task<string> ExecuteRequestStartTransaction20(ChargePointStatus chargePointStatus, OCPPCoreContext dbContext, string urlConnectorId, string idTag)
        {
            ...
            if (idTokenInfo.Status == AuthorizationStatusEnumType.Accepted)
            {
                ...
                requestStartTransactionRequest.IdToken.Type = ResolveRemoteStartIdTokenType20(logger);
                requestStartTransactionRequest.IdToken.IdToken = idTag;

                logger.LogInformation(
                    "OCPPMiddleware.OCPP20 => RequestStartTransaction20: ChargePoint='{0}' / ConnectorId={1} / idTag='{2}' / idTokenType='{3}'",
        ...
        {
            string apiResult = await ExecuteRequestStopTransaction20(chargePointStatus, dbContext, urlConnectorId, transactionId);
        ...
        private async Task<string> ExecuteRequestStopTransaction20(ChargePointStatus chargePointStatus, OCPPCoreContext dbContext, string urlConnectorId, string transactionId)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP20");
            ControllerOCPP20 controller20 = new ControllerOCPP20(_configuration, _logFactory, chargePointStatus, dbContext);
