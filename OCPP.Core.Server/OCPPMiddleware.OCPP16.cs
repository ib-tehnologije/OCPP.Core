...
namespace OCPP.Core.Server
{
    public partial class OCPPMiddleware
    {
        ...
        /// <summary>
        /// Send a ReserveNow-Request to the chargepoint (best effort).
        /// </summary>
        private async Task<string> ExecuteReserveNow16(ChargePointStatus chargePointStatus, OCPPCoreContext dbContext, int connectorId, string idTag, int reservationId, DateTime expiryUtc)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            ControllerOCPP16 controller16 = new ControllerOCPP16(_configuration, _logFactory, chargePointStatus, dbContext);

            var reserveRequest = new Messages_OCPP16.ReserveNowRequest
            {
            ...
            logger.LogInformation("OCPPMiddleware.OCPP16 => ReserveNow: ChargePoint='{0}' / ConnectorId={1} / IdTag='{2}' / Expiry={3:u} / ReservationId={4}",
            ...
            string jsonRequest = JsonConvert.SerializeObject(reserveRequest);
            ...
            {
                logger.LogInformation("OCPPMiddleware.OCPP16 => ReserveNow: Timeout (ChargePoint='{0}' / ConnectorId={1} / idTag='{2}')", chargePointStatus.Id, connectorId, idTag);
            }
        ...
        /// <summary>
        /// Send a CancelReservation-Request to the chargepoint (best effort).
        /// </summary>
        private async Task<string> ExecuteCancelReservation16(ChargePointStatus chargePointStatus, OCPPCoreContext dbContext, int reservationId)
        {
            ILogger logger = _logFactory.CreateLogger("OCPPMiddleware.OCPP16");
            ControllerOCPP16 controller16 = new ControllerOCPP16(_configuration, _logFactory, chargePointStatus, dbContext);

            var cancelRequest = new Messages_OCPP16.CancelReservationRequest
            {
                ReservationId = reservationId
            };
            ...
            {
                logger.LogInformation("OCPPMiddleware.OCPP16 => CancelReservation: Timeout (ChargePoint='{0}' / ReservationId={1})", chargePointStatus.Id, reservationId);
            }
