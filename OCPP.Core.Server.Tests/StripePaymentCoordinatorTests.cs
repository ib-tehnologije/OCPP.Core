...
namespace OCPP.Core.Server.Tests
{
    public class StripePaymentCoordinatorTests
    {
        ...
        private static StripePaymentCoordinator CreateCoordinator(
            ...
            PaymentFlowOptions? flowOptions = null,
        ...
        [Fact]
        public void IdleFeeCalculator_UsesZagrebLocalTimeForDenisTransaction4497Window()
        {
            TimeZoneInfo zagreb = ResolveZagrebTimeZone();
            DateTime intervalStartUtc = LocalToUtc(zagreb, new DateTime(2026, 5, 16, 22, 6, 0));
            DateTime intervalEndUtc = LocalToUtc(zagreb, new DateTime(2026, 5, 17, 8, 14, 0));
            ...
            Assert.Equal(14, minutes);
            Assert.Equal(20, intervalStartUtc.Hour);
            Assert.Equal(6, intervalEndUtc.Hour);
        }
        ...
        public void CompleteReservation_ReviewRequiredMeterEvidence_DoesNotCaptureInvoiceOrNotify()
        {
            ...
            {
                ReservationId = Guid.NewGuid(),
                ChargePointId = "CP-REVIEW-METER",
                ConnectorId = 1,
                ChargeTagId = "TAG-REVIEW-METER",
                StripePaymentIntentId = "pi_review_meter",
                Status = PaymentReservationStatus.Charging,
                PricePerKwh = 0.50m,
                Currency = "eur"
            };
            var transaction = new Transaction
            {
                ...
                AcceptedMeterKwh = 17.30859375,
                AcceptedMeterAtUtc = new DateTime(2026, 9, 10, 10, 0, 13, DateTimeKind.Utc),
                MeterEvidenceState = MeterEvidenceSettlementState.ReviewRequired,
                MeterEvidenceReason = MeterEvidenceReason.PhysicalCapacityUnavailable,
                MaxEnergyKwh = 80
            };
        ...
        public void CompleteReservation_CancelsBelowMinimumKwhEvenWhenEnergyExceedsMinimumCharge()
        {
            ...
            var transaction = new Transaction
            {
                ...
                ChargePointId = "CP-MIN-FEE",
                ConnectorId = 1,
                StartTagId = "TAG-MIN-FEE",
                StartTime = new DateTime(2026, 7, 5, 12, 0, 0, DateTimeKind.Utc),
        ...
        [Fact]
        public void CompleteReservation_CancelsSessionFeeOnlyCharge_ForLegitimateZeroEnergy()
        {
            CompleteReservationWithUnusableEnergy(meterStart: null, meterStop: 0, expectedStatus: PaymentReservationStatus.Cancelled, expectCancellation: true);
        }
        ...
        public void CompleteReservation_RequiresReview_WhenTerminalMeterHasNoAcceptedFallback(double? meterStart, double? meterStop)
        {
            CompleteReservationWithUnusableEnergy(meterStart, meterStop, PaymentReservationStatus.ReviewRequired, expectCancellation: false);
        }

        private void CompleteReservationWithUnusableEnergy(
            double? meterStart,
            double? meterStop,
            ...
            bool expectCancellation)
        {
            ...
            {
                ReservationId = Guid.NewGuid(),
                ChargePointId = "CP-NO-ENERGY",
                ConnectorId = 1,
                ChargeTagId = "TAG-NO-ENERGY",
                StripePaymentIntentId = "pi_no_energy",
                Status = PaymentReservationStatus.Charging,
                PricePerKwh = 0.50m,
                UserSessionFee = 0.50m,
                UsageFeePerMinute = 0m,
            ...
            var intentService = new FakePaymentIntentService
            {
                GetResponse = new PaymentIntent { Id = "pi_no_energy", Status = "requires_capture", Amount = 10_000 }
            };
            var invoiceIntegration = new FakeInvoiceIntegrationService();
            var coordinator = CreateCoordinator(
                context,
            ...
            Assert.Equal(expectCancellation, intentService.CancelCalled);
            Assert.False(intentService.CaptureCalled);
            Assert.Equal(0m, transaction.UserSessionFeeAmount);
            Assert.Equal(0, invoiceIntegration.HandleCompletedReservationCount);
        }
        ...
        [Fact]
        public void CompleteReservation_KeepsSessionFeeAtConfiguredMinimumKwh()
        {
            using var context = CreateContext();
            var reservation = new ChargePaymentReservation
            {
                ReservationId = Guid.NewGuid(),
                ChargePointId = "CP-MIN-ALLOW",
                ConnectorId = 1,
                ChargeTagId = "TAG-MIN-ALLOW",
                StripePaymentIntentId = "pi_min_allow",
                Status = PaymentReservationStatus.Charging,
                PricePerKwh = 0.50m,
                UserSessionFee = 0.50m,
                UsageFeePerMinute = 0m,
            ...
            var intentService = new FakePaymentIntentService
            {
                GetResponse = new PaymentIntent { Id = "pi_min_allow", Status = "requires_capture", Amount = 10_000 }
            };
            var coordinator = CreateCoordinator(
                context,
                new FakeSessionService(),
            ...
            Assert.Equal(PaymentReservationStatus.Completed, reservation.Status);
            Assert.True(intentService.CaptureCalled);
            Assert.Equal(100, intentService.LastCaptureOptions?.AmountToCapture ?? 0);
            Assert.Equal(0.50m, transaction.UserSessionFeeAmount);
        }
        ...
        [Fact]
        public void CompleteReservation_CompletesWhenCircontrolReportsAvailableBeforeStopTransaction()
        {
            using var context = CreateContext();
            var stopAtUtc = new DateTime(2026, 3, 12, 12, 20, 0, DateTimeKind.Utc);
            var availableAtUtc = stopAtUtc.AddSeconds(-3);
            var reservation = new ChargePaymentReservation
            {
                ReservationId = Guid.NewGuid(),
                ChargePointId = "CP-CIRCONTROL",
                ConnectorId = 1,
                ChargeTagId = "TAG-CIRCONTROL",
                StripePaymentIntentId = "pi_circontrol",
                Status = PaymentReservationStatus.Charging,
                PricePerKwh = 0.50m,
                UserSessionFee = 0.50m,
                UsageFeePerMinute = 0.20m,
            ...
            context.ConnectorStatuses.Add(new ConnectorStatus
            {
                ...
                LastStatusTime = availableAtUtc
            });
            context.SaveChanges();

            var intentService = new FakePaymentIntentService
            {
                GetResponse = new PaymentIntent { Id = "pi_circontrol", Status = "requires_capture", Amount = 10_000 }
            };
            var coordinator = CreateCoordinator(
                context,
                new FakeSessionService(),
                intentService,
                now: () => stopAtUtc.AddSeconds(5));

            coordinator.CompleteReservation(context, transaction);

            Assert.Equal(PaymentReservationStatus.Completed, reservation.Status);
            Assert.Equal(availableAtUtc, reservation.DisconnectedAtUtc);
            Assert.True(intentService.CaptureCalled);
            Assert.False(intentService.CancelCalled);
        }
